#!/usr/bin/env python3
"""Generate the Swedish bootstrap locale from the pinned English audit payloads.

The translation cache is kept under build/ so interrupted runs can resume. This is
intended as a bootstrap pass: the generated locale still benefits from native review.
"""
...
CACHE_PATH = ROOT / "build/swedish_translation_cache.json"
TOKEN_RE = re.compile(r"%(?:\d+\$)?[sdif]|§.|\\n|\{[^{}]+\}|<[^<>]+>")
PLACEHOLDER_RE = re.compile(r"%(?:\d+\$)?[sdif]")
SEPARATOR_RE = re.compile(r"\n?ZXQSEP\d{4}ZXQ\n?")
MANUAL_OVERRIDES = {
    "Open Origin Creator": "Öppna Origin-skaparen",
    "Mob Origin Creator": "Mob-Origin-skapare",
    "Split": "Dela",
    "Elytra Boost": "Elytraförstärkning",
    "Shulker Bulk": "Shulkerstyrka",
    "Sonic Boom": "Ljudbang",
    "Pack Bond": "Flockband",
    "Elytrian Elytra Boost": "Elytrians elytraförstärkning",
    "Hiveling Sting": "Hiveling-gadd",
    "Piglin Attack Bonus": "Piglin-attackbonus",
    "Sculkborn Sonic Bolt": "Sculkborn-ljudprojektil",
    "Umbral Shadow Orb": "Umbral-skuggklot",
    "Sky Piercer": "Himmelsgenomborrare",
    "Apex Sky Piercer": "Apex-himmelsgenomborrare",
    "Lilliputian Dodge": "Lilliputiansk undanmanöver",
    "NeoOrigins: Grant Loot Pool": "NeoOrigins: Tilldela loot-pool",
    "Rage Counter.": "Raserimätare.",
    "Rage Counter": "Raserimätare",
    "Speed Mining": "Snabb grävning",
    "Camoflauge": "Kamouflage",
    "Pack Boost": "Flockförstärkning",
    "Max Mana Boost": "Förstärkning av maximal mana",
}
...
def translate_values(values: list[str]):
    ...
    print(f"Swedish cache: {len(cache)} entries; {len(pending)} new strings in {len(batches)} batches")
...
def refine_identical_titles(cache: dict):
    ...
    for source in candidates:
        ...
        changed += 1
    write_json(CACHE_PATH, cache)
    print(f"Refined {changed}/{len(candidates)} title-like strings that were left in English")
    return cache


def main():
    neo_121 = read_json(ROOT / "build/sv-discovery-mc-1.21.1/sv_se_missing_en.json")
    neo_261 = read_json(ROOT / "build/sv-discovery-mc-26.1/sv_se_missing_en.json")
    neo_262 = read_json(ROOT / "build/sv-discovery-mc-26.2/sv_se_missing_en.json")
    ...
    if (len(common), len(delta_121), len(delta_261), len(delta_262)) != (2290, 6, 17, 17):
        raise RuntimeError(
            "Unexpected NeoOrigins split: "
    ...
    for index in range(16):
        chunk = dict(common_items[index * 150 : (index + 1) * 150])
        write_json(ASSETS / f"neoorigins_sv_common_{index + 1:02d}/lang/sv_se.json", translated(chunk))
    write_json(ASSETS / "neoorigins_sv_121/lang/sv_se.json", translated(delta_121))
    write_json(ASSETS / "neoorigins_26_1/lang/sv_se.json", translated(delta_261))
    write_json(ASSETS / "neoorigins_26_2/lang/sv_se.json", translated(delta_262))
    for namespace, payload in addons.items():
        write_json(ASSETS / namespace / "lang/sv_se.json", translated(payload))

    print(
        "Generated Swedish locale: "
