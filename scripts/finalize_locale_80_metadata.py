...
LOCALE = "brb"
DISPLAY_NAME = "Braobans"
SOURCE_LOCALE = "bar"


def rewrite_locale_strings(value):
    """Clone Bavarian metadata without replacing incidental 'bar' substrings."""
    ...
    if isinstance(value, str):
        if value == SOURCE_LOCALE:
            return LOCALE
        value = value.replace("neoorigins_bar_", "neoorigins_brb_")
        value = re.sub(r"(?<=/)bar(?=\.json(?:$|[?#]))", LOCALE, value)
        value = re.sub(r"(?<=\\)bar(?=\.json(?:$|[?#]))", LOCALE, value)
        return value
    return value


def add_brabantian_from_bavarian(node) -> int:
    added = 0
    ...
    def walk(value):
        ...
        if isinstance(value, dict):
            if SOURCE_LOCALE in value and isinstance(value[SOURCE_LOCALE], dict):
                if LOCALE in value:
                    raise SystemExit("Brabantian metadata already exists before locale 80 finalization")
...
def update_supported_counts(node) -> int:
    ...
    if isinstance(node, dict):
        for key, value in node.items():
            if key == "supported_locale_count" and value == 79:
                node[key] = 80
                changed += 1
            else:
...
def assert_brabantian_entry_clean(value, project_id: str) -> None:
    if isinstance(value, dict):
        for item in value.values():
            assert_brabantian_entry_clean(item, project_id)
    elif isinstance(value, list):
        for item in value:
            assert_brabantian_entry_clean(item, project_id)
    elif isinstance(value, str):
        if "/bar.json" in value or "neoorigins_bar_" in value:
            raise SystemExit(
                f"{project_id}: Bavarian path/namespace leaked into Brabantian metadata: {value!r}"
            )
...
def main() -> None:
    ...
    if project.get("supported_locale_count") != 79:
        raise SystemExit(
            "Expected catalog supported_locale_count=79 before Brabantian finalization, "
            f"got {project.get('supported_locale_count')!r}"
        )

    added = add_brabantian_from_bavarian(catalog)
    if added != 11:
        raise SystemExit(f"Expected 11 Brabantian catalog entries, got {added}")
    ...
    additions = {
        "brabantian_common_glob": "neoorigins_brb_common_*",
        "brabantian_mc_1_21_1": "neoorigins_brb_121",
    }
    ...
    if "All 79 currently supported locales" not in old_note:
        raise SystemExit("Unexpected NeoOrigins coverage_note before locale 80 finalization")
    ...
    readme = replace_exact(readme, "| 79 |", "| 80 |", expected=3)
    readme = replace_exact(
        readme,
        "**Bavarois (`bar`)**.",
        "**Bavarois (`bar`)** et **Brabançon (`brb`)**.",
        expected=1,
    )
    readme = replace_exact(
        readme,
    ...
    brabantian_block = bavarian_block + (
        "\n\nPour le brabançon :\n\n"
        "- **1.21.1 : 2 296/2 296 clés** couvertes ;\n"
        "- **26.1.x : 2 307/2 307 clés** couvertes ;\n"
        "- **26.2 : 2 307/2 307 clés** couvertes."
    )
    readme = replace_exact(readme, bavarian_block, brabantian_block, expected=1)
    ...
    catalog_md = replace_exact(
        catalog_md,
        "Boarisch (`bar`)",
        "Boarisch (`bar`) · Braobans (`brb`)",
        expected=11,
    )
    CATALOG_MD.write_text(catalog_md, encoding="utf-8")

    final_catalog = json.loads(CATALOG_JSON.read_text(encoding="utf-8"))
    if final_catalog["project"]["supported_locale_count"] != 80:
        raise SystemExit("Final supported_locale_count is not 80")
    for supported_project in final_catalog["supported_projects"]:
        ...
        if languages[LOCALE].get("name") != DISPLAY_NAME:
            raise SystemExit(
                f"{project_id}: unexpected Brabantian display name "
                f"{languages[LOCALE].get('name')!r}"
            )
        assert_brabantian_entry_clean(languages[LOCALE], project_id)

    print("Metadata finalization passed: 79 -> 80 locales; brb added to all 11 projects.")
