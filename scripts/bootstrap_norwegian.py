#!/usr/bin/env python3
"""Generate Norwegian Bokmål fallback localization from pinned English audit payloads.

The translation cache is kept under build/ so interrupted CI runs can resume within
one job. Official upstream Norwegian strings are pruned by the audit pass after this
...
CACHE_PATH = ROOT / "build/norwegian_translation_cache.json"
...
MANUAL_OVERRIDES = {
    "Open Origin Creator": "Åpne Origin-oppretteren",
    "Mob Origin Creator": "Mob Origin-oppretter",
    "Split": "Del",
    "Elytra Boost": "Elytra-forsterkning",
    "Sonic Boom": "Sonisk drønn",
    "Pack Bond": "Flokkbånd",
    "Speed Mining": "Rask gruvedrift",
    "Camoflauge": "Kamuflasje",
    "Pack Boost": "Flokkforsterkning",
    "Max Mana Boost": "Maks mana-forsterkning",
...
def translate_rpc(text: str, attempts: int = 5):
    # Google Translate uses "no" for Norwegian Bokmål.
    inner = json.dumps([[text, "en", "no", True], [None]])
...
def translate_values(values: list[str]):
    ...
    print(f"Norwegian cache: {len(cache)} entries; {len(pending)} new strings in {len(batches)} batches")
...
def main():
    neo_121 = read_json(ROOT / "build/no-discovery-mc-1.21.1/no_no_missing_en.json")
    neo_261 = read_json(ROOT / "build/no-discovery-mc-26.1/no_no_missing_en.json")
    neo_262 = read_json(ROOT / "build/no-discovery-mc-26.2/no_no_missing_en.json")
    ...
    for index in range(common_chunks):
        ...
        write_json(ASSETS / f"neoorigins_no_common_{index + 1:02d}/lang/no_no.json", translated(chunk))

    write_json(ASSETS / "neoorigins_no_121/lang/no_no.json", translated(delta_121))
    write_json(ASSETS / "neoorigins_26_1/lang/no_no.json", translated(delta_261))
    write_json(ASSETS / "neoorigins_26_2/lang/no_no.json", translated(delta_262))
    for namespace, payload in addons.items():
        write_json(ASSETS / namespace / "lang/no_no.json", translated(payload))

    print(
        "Generated Norwegian locale: "
