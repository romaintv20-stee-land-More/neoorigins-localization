#!/usr/bin/env python3
"""Refine Limburgish (`li_li`) with direct NLLB English -> Limburgish MT."""
...
MODEL_ID="facebook/nllb-200-distilled-600M"; SOURCE_LANG="eng_Latn"; TARGET_LANG="lim_Latn"
...
def build_sources():
    n121=read_json(ROOT/"build/li-discovery-mc-1.21.1/li_li_missing_en.json"); n261=read_json(ROOT/"build/li-discovery-mc-26.1/li_li_missing_en.json"); n262=read_json(ROOT/"build/li-discovery-mc-26.2/li_li_missing_en.json")
...
def main():
    before_files=sorted(ASSETS.glob("**/lang/li_li.json"))
    if len(before_files)!=29: raise SystemExit(f"Expected 29 Limburgish files, found {len(before_files)}")
    ...
    print(f"Limburgish pool: {len(unique)} unique; {len(unique)-len(direct)} corpus/manual; {len(direct)} direct NLLB",flush=True)
    mt=Translator()
    for start in range(0,len(direct),24):
        batch=direct[start:start+24]; outputs=mt.batch(batch)
        if len(outputs)!=len(batch): raise SystemExit("NLLB batch size mismatch")
        for source,value in zip(batch,outputs):
            ...
            if not value: raise SystemExit(f"Empty Limburgish translation: {source!r}")
            if base.placeholder_signature(source)!=base.placeholder_signature(value): raise SystemExit(f"Placeholder mismatch: {source!r}->{value!r}")
            if protected_signature(source)!=protected_signature(value): raise SystemExit(f"Protected-token mismatch: {source!r}->{value!r}")
            if SUSPICIOUS_RE.search(value): raise SystemExit(f"Suspicious Limburgish output: {source!r}->{value!r}")
            translated[source]=value
        done=min(start+len(batch),len(direct))
        if done%240==0 or done==len(direct): print(f"Direct Limburgish progress: {done}/{len(direct)}",flush=True)
    def pl(src):return {k:translated[str(v)] for k,v in src.items()}
    common_li=pl(common)
    for p in ASSETS.glob("neoorigins_li_common_*/lang/li_li.json"):p.unlink()
    items=list(common_li.items())
    for i in range((len(items)+149)//150): write_json(ASSETS/f"neoorigins_li_common_{i+1:02d}/lang/li_li.json",dict(items[i*150:(i+1)*150]))
    write_json(ASSETS/"neoorigins_li_121/lang/li_li.json",pl(d121)); write_json(ASSETS/"neoorigins_26_1/lang/li_li.json",pl(d261)); write_json(ASSETS/"neoorigins_26_2/lang/li_li.json",pl(d262))
    for ns,src in addons.items():write_json(ASSETS/ns/"lang/li_li.json",pl(src))
    after_files=sorted(ASSETS.glob("**/lang/li_li.json")); after={p:read_json(p) for p in after_files}; seen={}; changed=total=0
    if len(after_files)!=29: raise SystemExit("Limburgish file count changed")
    for p,data in after.items():
        if set(before[p])!=set(data): raise SystemExit(f"Key set changed: {p}")
    ...
    if TARGET_LANG in text: raise SystemExit("NLLB marker survived")
    if source_changed<2500: raise SystemExit(f"Too many English values survived: {source_changed}")
    ...
    print(f"Limburgish refinement passed: {total} values / 29 files; {changed} bootstrap changes; {source_changed} source values changed",flush=True)
