...
DEFAULT_FILE_ID = "8397352"
DEFAULT_FILENAME = "Origins-Furries-1.21.1-NeoOrigins-1.0.0.jar"
...
def download_jar(file_id: str, filename: str, attempts: int = 4):
    ...
    urls = (
        ...
        f"https://www.curseforge.com/minecraft/mc-mods/origins-furries-for-neoorigins/download/{file_id}/file",
    )
    last_error = None
    for attempt in range(1, attempts + 1):
        ...
        if attempt != attempts:
            delay = 2 ** (attempt - 1)
            print(f"Transient Origins Furries download error ({attempt}/{attempts}): {last_error}; retrying in {delay}s")
            time.sleep(delay)
    raise last_error
...
def detect_namespace(jar: zipfile.ZipFile, preferred: str):
    ...
    if len(candidates) == 1:
        return candidates[0]
    raise SystemExit(f"Origins Furries audit failed: could not uniquely detect localization namespace; candidates={candidates}")
...
def main():
    parser = argparse.ArgumentParser(description="Compare Origins Furries for NeoOrigins fallback locales with the pinned upstream CurseForge JAR.")
    parser.add_argument("--file-id", default=DEFAULT_FILE_ID)
    parser.add_argument("--filename", default=DEFAULT_FILENAME)
    parser.add_argument("--namespace", default=DEFAULT_NAMESPACE)
    parser.add_argument("--output", default=str(ROOT / "build/origins-furries-upstream-audit"))
    ...
    with zipfile.ZipFile(io.BytesIO(jar_data)) as jar:
        ...
        if not en:
            raise SystemExit("Origins Furries audit failed: upstream en_us.json is missing")
    ...
    report = {
        "upstream": "Origins Furries for NeoOrigins",
        "curseforge_project_id": 1592095,
        ...
        if args.annotate_missing:
            for key in missing:
                if key not in annotation_keys:
                    ...
                    print(f"::error title=Missing Origins Furries translation::{escape_annotation(key + ' = ' + str(en.get(key, '')))}")

    (out_dir / "report.json").write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"Origins Furries for NeoOrigins audit ({args.filename}, CurseForge file {args.file_id})")
    ...
    if failures:
        raise SystemExit("Origins Furries audit failed: " + "; ".join(failures))
