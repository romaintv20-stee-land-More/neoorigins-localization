...
if catalog['project']['supported_locale_count'] != 65:
    raise SystemExit(f"Expected 65 locales before Albanian, got {catalog['project']['supported_locale_count']}")
...
def walk(obj):
    ...
    if isinstance(obj, dict):
        if 'ta_in' in obj and 'sq_al' not in obj and isinstance(obj['ta_in'], dict) and 'name' in obj['ta_in']:
            entry = deepcopy(obj['ta_in'])
            entry['name'] = 'Shqip'
            if isinstance(entry.get('file'), str):
                entry['file'] = entry['file'].replace('/ta_in.json', '/sq_al.json')
            obj['sq_al'] = entry
            added += 1
...
ns['albanian_common_glob'] = 'neoorigins_sq_common_*'
ns['albanian_mc_1_21_1'] = 'neoorigins_sq_121'
...
if added < 11:
    raise SystemExit(f'Expected Albanian language entries for NeoOrigins + 10 add-ons, got {added}')
...
text = text.replace(needle, 'தமிழ் (`ta_in`) · Shqip (`sq_al`)')
...
new = '**Somali (`so_so`)**, **Espéranto (`eo_uy`)**, **Kirghize (`ky_kg`)**, **Tamoul (`ta_in`)** et **Albanais (`sq_al`)**.'
if old not in text:
...
text = text.replace(ta_block, sq_block, 1)
p.write_text(text, encoding='utf-8')

print(f'Albanian metadata prepared: 66 locales; {added} catalog language entries; {hits} catalogue rows.')
