...
def download_jar(file_id: str, filename: str, attempts: int = 4):
    ...
    urls = (
        ...
        f"https://www.curseforge.com/minecraft/mc-mods/origins-backgrounds-iss-for-neoorigins/download/{file_id}/file",
    )
    last_error = None
    for attempt in range(1, attempts + 1):
        ...
        if attempt != attempts:
            delay = 2 ** (attempt - 1)
            print(f"Transient Origins Backgrounds ISS download error ({attempt}/{attempts}): {last_error}; retrying in {delay}s")
            time.sleep(delay)
    raise last_error
...
def main():
    parser = argparse.ArgumentParser(description="Compare Origins: Backgrounds ISS for NeoOrigins fallback locales with the pinned upstream CurseForge JAR.")
    parser.add_argument("--file-id", default=DEFAULT_FILE_ID)
    parser.add_argument("--filename", default=DEFAULT_FILENAME)
    parser.add_argument("--namespace", default=DEFAULT_NAMESPACE)
    parser.add_argument("--output", default=str(ROOT / "build/origins-backgrounds-iss-upstream-audit"))
    ...
    with zipfile.ZipFile(io.BytesIO(jar_data)) as jar:
        ...
        if not en:
            raise SystemExit("Origins Backgrounds ISS audit failed: upstream en_us.json is missing")
        ...
        if args.annotate_missing:
            for key in missing:
                if key not in annotation_keys:
                    ...
                    print(f"::error title=Missing Origins Backgrounds ISS translation::{escape_annotation(key + ' = ' + str(en.get(key, '')))}")
    ...
    if failures:
        raise SystemExit("Origins Backgrounds ISS audit failed: " + "; ".join(failures))
