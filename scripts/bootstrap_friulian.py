#!/usr/bin/env python3
"""Generate Friulian (`fur_it`) fallback localization.

The semantic source is the project's complete Italian coverage, with official NeoOrigins
Italian translations taking precedence when they exist. Friulian projection is learned
from the pinned Minecraft Java `it_it` <-> `fur_it` parallel corpus. Suspicious,
untranslated, placeholder-breaking and non-Latin corpus pairs are rejected. This is a
deterministic bootstrap, not a native-speaker review.
"""
...
DIALECT_MARKER_RE = re.compile(
    r"\b(?:servidôr|comant|abilitât|disabilitât|cjape|cjat|cjatar|ducj|jessi|vuelistu|plui|cun|une|par|dal|dai|no|son|fâs|fûc|mont|paîs|gjenar|gjeneradôr)\b",
    re.IGNORECASE,
)
...
# Very common forms repeatedly visible in the pinned Friulian Minecraft corpus.
# Statistical evidence remains the primary source; these only ensure basic grammar
# differentiation where isolated alignment can be sparse.
MANUAL_WORDS = {
    "non": "no",
    "sono": "son",
    "con": "cun",
...
def fetch_json(url: str):
    request = urllib.request.Request(url, headers={"User-Agent": "NeoOrigins-Friulian-Bootstrap"})
...
def safe_pair(source: str, target: str, english: str | None) -> bool:
    ...
    if placeholder_signature(source) != placeholder_signature(target):
        return False
    # Community locales can contain newer entries still left in English. Never let an
    # untranslated English value teach the Italian -> Friulian projector.
    ...
    return similarity >= 0.20 or bool(DIALECT_MARKER_RE.search(target))
...
def build_corpus_maps():
    italian = fetch_json(f"{MC_LOCALES_BASE}/it_it.json")
    friulian = fetch_json(f"{MC_LOCALES_BASE}/fur_it.json")
    english = fetch_json(f"{MC_LOCALES_BASE}/en_us.json")
    shared = sorted(set(italian) & set(friulian))
    exact: dict[str, str] = {}
    ...
    for key in shared:
        source = str(italian[key])
        target = str(friulian[key])
    ...
    for source, counter in votes.items():
        ...
        # Friulian and Italian are related but not interchangeable. Require repeated,
        # dominant evidence and reject radical one-off projections.
        if len(target) >= 2 and similarity >= 0.25 and (
    ...
    destructive = {
        ...
        if len(source) >= 5 and (len(target) <= 1 or SequenceMatcher(None, source, target).ratio() < 0.18)
    }
    if destructive:
        raise RuntimeError(f"Destructive Friulian mappings detected: {destructive}")

    learned.update(MANUAL_WORDS)
    print(
        f"Minecraft Friulian corpus: {len(shared)} aligned entries, "
        f"{rejected} rejected suspicious/untranslated pairs, {unchanged} unchanged pairs, "
        f"{len(exact)} exact Friulian strings, {len(learned)} learned word mappings"
    )
    return exact, learned


def friulianize(text: str, exact: dict[str, str], learned: dict[str, str]) -> str:
    ...
    if placeholder_signature(text) != placeholder_signature(result):
        raise RuntimeError(f"Friulian placeholder mismatch: {text!r} -> {result!r}")
    return result


def collect_local_italian() -> dict[str, str]:
    merged: dict[str, str] = {}
    files = sorted(ASSETS.glob("**/lang/it_it.json"))
    for path in files:
        for key, value in read_json(path).items():
            merged.setdefault(key, value)
    print(f"Local Italian fallback pool: {len(merged)} distinct keys from {len(files)} files")
    return merged


def neo_italian(ref: str, english: dict[str, str], local_italian: dict[str, str]) -> dict[str, str]:
    official = fetch_optional_json(NEO_BASE.format(ref=ref, locale="it_it"))
    merged = dict(official)
    for key, value in local_italian.items():
        merged.setdefault(key, value)
    missing = sorted(set(english) - set(merged))
    if missing:
        raise RuntimeError(
            f"Italian NeoOrigins semantic source incomplete at {ref}: {len(missing)} missing, sample={missing[:20]}"
        )
    print(f"Italian semantic source {ref}: official={len(official)}, merged={len(merged)}")
    return {key: str(merged[key]) for key in english}


def addon_italian(namespace: str, english: dict[str, str], local_italian: dict[str, str]) -> dict[str, str]:
    merged = dict(local_italian)
    direct = ASSETS / namespace / "lang/it_it.json"
    if direct.exists():
        merged.update(read_json(direct))
    missing = sorted(set(english) - set(merged))
    if missing:
        raise RuntimeError(f"Italian source incomplete for {namespace}: {len(missing)} missing, sample={missing[:20]}")
    return {key: str(merged[key]) for key in english}


def translated_payload(source: dict[str, str], exact, learned):
    return {key: friulianize(value, exact, learned) for key, value in source.items()}


def main():
    neo_121_en = read_json(ROOT / "build/fur-discovery-mc-1.21.1/fur_it_missing_en.json")
    neo_261_en = read_json(ROOT / "build/fur-discovery-mc-26.1/fur_it_missing_en.json")
    neo_262_en = read_json(ROOT / "build/fur-discovery-mc-26.2/fur_it_missing_en.json")
    ...
    print(
        f"NeoOrigins Friulian split: common={len(common_en)}, "
    ...
    local_italian = collect_local_italian()
    exact, learned = build_corpus_maps()
    it_121 = neo_italian(NEO_REFS["mc-1.21.1"], neo_121_en, local_italian)
    it_261 = neo_italian(NEO_REFS["mc-26.1"], neo_261_en, local_italian)
    it_262 = neo_italian(NEO_REFS["mc-26.2"], neo_262_en, local_italian)

    disagreements = [
        key for key in common_en
        if it_121.get(key) != it_261.get(key) or it_121.get(key) != it_262.get(key)
    ]
    print(f"Common Italian target disagreements: {len(disagreements)}")

    common_fur = translated_payload({k: it_121[k] for k in common_en}, exact, learned)
    delta_121_fur = translated_payload({k: it_121[k] for k in delta_121_en}, exact, learned)
    delta_261_fur = translated_payload({k: it_261[k] for k in delta_261_en}, exact, learned)
    delta_262_fur = translated_payload({k: it_262[k] for k in delta_262_en}, exact, learned)

    for path in ASSETS.glob("neoorigins_fur_common_*/lang/fur_it.json"):
        path.unlink()

    common_items = list(common_fur.items())
    chunk_size = 150
    chunks = (len(common_items) + chunk_size - 1) // chunk_size
    for index in range(chunks):
        write_json(
            ASSETS / f"neoorigins_fur_common_{index + 1:02d}/lang/fur_it.json",
            dict(common_items[index * chunk_size:(index + 1) * chunk_size]),
        )
    write_json(ASSETS / "neoorigins_fur_121/lang/fur_it.json", delta_121_fur)
    write_json(ASSETS / "neoorigins_26_1/lang/fur_it.json", delta_261_fur)
    write_json(ASSETS / "neoorigins_26_2/lang/fur_it.json", delta_262_fur)

    for namespace, english in addons_en.items():
        italian = addon_italian(namespace, english, local_italian)
        write_json(ASSETS / namespace / "lang/fur_it.json", translated_payload(italian, exact, learned))

    files = sorted(ASSETS.glob("**/lang/fur_it.json"))
    if len(files) != 29:
        raise RuntimeError(f"Expected 29 Friulian files after generation, found {len(files)}")
    ...
    marked_values = sum(1 for value in all_values if DIALECT_MARKER_RE.search(value))
    if marker_count < 150 or marked_values < 120:
        raise RuntimeError(
            f"Friulian differentiation too weak: markers={marker_count}, marked_values={marked_values}"
        )
    if any(unsupported_script(value) for value in all_values):
        raise RuntimeError("Unexpected non-Latin script found in Friulian output")
    print(
        f"Friulian bootstrap generated {len(files)} files; "
