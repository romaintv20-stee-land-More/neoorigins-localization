#!/usr/bin/env python3
"""Refine Mohawk (`moh_ca`) fallbacks with direct English -> Mohawk OPUS MT."""
from pathlib import Path
import json,re,torch
from transformers import AutoModelForSeq2SeqLM,AutoTokenizer
import bootstrap_mohawk as base
ROOT=base.ROOT;ASSETS=base.ASSETS;MODEL_ID="Helsinki-NLP/opus-mt-en-mul";TARGET_TOKEN=">>moh<<"
...
def protected_signature(t):return PROTECT_RE.findall(t)
def sanitize(s,v):
...
class MT:
 def __init__(self):self.t=AutoTokenizer.from_pretrained(MODEL_ID);self.m=AutoModelForSeq2SeqLM.from_pretrained(MODEL_ID);self.m.eval();print(f"Direct Mohawk OPUS: {MODEL_ID} {TARGET_TOKEN}",flush=True)
 def batch(self,x):
  if not x:return []
...
def main():
 ...
 print(f"Mohawk pool: {len(unique)} unique; {len(unique)-len(direct)} corpus/manual; {len(direct)} direct OPUS",flush=True);mt=MT()
 for start in range(0,len(direct),24):
  ...
  if done%240==0 or done==len(direct):print(f"Direct Mohawk progress: {done}/{len(direct)}",flush=True)
 ...
 if TARGET_TOKEN in text or source_changed<2500 or seen.get("key.categories.originsmodernui")!="Origin Architect":raise SystemExit("Mohawk final semantic/coverage gate failed")
