...
LOCALES = ("fr_fr", "de_de", "es_es", "pt_br", "nl_nl", "it_it", "pl_pl", "ru_ru", "tr_tr", "zh_cn", "cs_cz", "hu_hu", "ja_jp", "ko_kr", "uk_ua", "id_id")
DEFAULT_FILE_ID = "8393343"
DEFAULT_FILENAME = "Origins-Classes-Ex-1.21.1-NeoOrigins-1.0.1.jar"
DEFAULT_NAMESPACE = "origins_classes_extended"
...
def download_jar(file_id: str, filename: str, attempts: int = 4):
    ...
    urls = (
        ...
        f"https://www.curseforge.com/minecraft/mc-mods/origins-classes-extended-for-neoorigins/download/{file_id}/file",
    )
    last_error = None
    for attempt in range(1, attempts + 1):
        ...
        if attempt != attempts:
            delay = 2 ** (attempt - 1)
            print(f"Transient Origins Classes Extended download error ({attempt}/{attempts}): {last_error}; retrying in {delay}s")
            time.sleep(delay)
    raise last_error
...
def detect_namespace(jar: zipfile.ZipFile, preferred: str):
    ...
    if len(candidates) == 1:
        return candidates[0]
    raise SystemExit(f"Origins Classes Extended audit failed: could not uniquely detect localization namespace; candidates={candidates}")
...
def main():
    parser = argparse.ArgumentParser(description="Compare Origins: Classes Extended for NeoOrigins fallback locales with the pinned upstream CurseForge JAR.")
    parser.add_argument("--file-id", default=DEFAULT_FILE_ID)
    parser.add_argument("--filename", default=DEFAULT_FILENAME)
    parser.add_argument("--namespace", default=DEFAULT_NAMESPACE)
    parser.add_argument("--output", default=str(ROOT / "build/origins-classes-extended-upstream-audit"))
    ...
    with zipfile.ZipFile(io.BytesIO(jar_data)) as jar:
        ...
        if not en:
            raise SystemExit("Origins Classes Extended audit failed: upstream en_us.json is missing")
    ...
    report = {
        "upstream": "Origins: Classes Extended for NeoOrigins",
        "curseforge_project_id": 1602395,
        ...
        if args.annotate_missing:
            for key in missing:
                if key not in annotation_keys:
                    ...
                    print(f"::error title=Missing Origins Classes Extended translation::{escape_annotation(key + ' = ' + str(en.get(key, '')))}")

    (out_dir / "report.json").write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"Origins: Classes Extended for NeoOrigins audit ({args.filename}, CurseForge file {args.file_id})")
    ...
    if failures:
        raise SystemExit("Origins Classes Extended audit failed: " + "; ".join(failures))
