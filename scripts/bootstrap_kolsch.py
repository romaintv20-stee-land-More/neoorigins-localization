#!/usr/bin/env python3
"""Generate Kölsch (`ksh`) fallback localization bootstrap files.

English remains the semantic source. Exact whole-string replacements are reused from
the pinned Minecraft Java `en_us` <-> `ksh` corpus. Remaining English strings are
left intact for a dedicated direct English -> Kölsch full-sentence refinement pass.
No Standard German pivot and no isolated-word projection are used.
This is automated assistance, not native-speaker review.
"""
...
def fetch_json(url: str):
    request = urllib.request.Request(url, headers={"User-Agent": "NeoOrigins-Kolsch-Bootstrap"})
...
def build_corpus_map() -> dict[str, str]:
    ...
    kolsch = fetch_json(f"{MC_LOCALES_BASE}/ksh.json")
    shared = sorted(set(english) & set(kolsch))
    exact_votes: dict[str, Counter[str]] = defaultdict(Counter)
    rejected = unchanged = 0
    for key in shared:
        source = str(english[key])
        target = str(kolsch[key])
        if source.casefold() == target.casefold():
            unchanged += 1
            continue
    ...
    for source, counter in exact_votes.items():
        ...
        if top / total >= 0.80:
            ...
            ambiguous += 1
    print(
        f"Minecraft Kölsch corpus: {len(shared)} aligned entries, {rejected} rejected pairs, "
        f"{unchanged} unchanged English pairs, {len(exact)} exact Kölsch strings, "
        f"{ambiguous} ambiguous exact sources"
    )
    return exact


def kolschize(text: str, exact: dict[str, str]) -> str:
    ...
    if placeholder_signature(text) != placeholder_signature(result):
        raise RuntimeError(f"Kölsch placeholder mismatch: {text!r} -> {result!r}")
    return result


def translated_payload(source: dict[str, str], exact: dict[str, str]):
    return {key: kolschize(str(value), exact) for key, value in source.items()}
...
def main():
    ...
    print(f"NeoOrigins Kölsch split: common={len(common_en)}, 1.21.1={len(delta_121_en)}, 26.1={len(delta_261_en)}, 26.2={len(delta_262_en)}")
    ...
    common_ksh = translated_payload(common_en, exact)
    delta_121_ksh = translated_payload(delta_121_en, exact)
    delta_261_ksh = translated_payload(delta_261_en, exact)
    delta_262_ksh = translated_payload(delta_262_en, exact)
    ...
    for index in range(chunks):
        write_json(ASSETS / f"neoorigins_ksh_common_{index + 1:02d}/lang/ksh.json", dict(common_items[index * chunk_size:(index + 1) * chunk_size]))
    ...
    for namespace, english in addons_en.items():
        write_json(ASSETS / namespace / "lang/ksh.json", translated_payload(english, exact))

    files = sorted(ASSETS.glob("**/lang/ksh.json"))
    if len(files) != 29:
        raise RuntimeError(f"Expected 29 Kölsch files after generation, found {len(files)}")
    ...
    print(
        f"Kölsch bootstrap generated {len(files)} files; exact/manual replacements applied to {exact_hits}/{total_values} source values; "
