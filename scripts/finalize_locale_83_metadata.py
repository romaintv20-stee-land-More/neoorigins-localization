...
LOCALE = "fur_it"
DISPLAY_NAME = "Furlan"
SOURCE_LOCALE = "fra_de"


def rewrite_locale_strings(value):
    """Clone East Franconian metadata while changing only locale paths/namespaces."""
    ...
    if isinstance(value, str):
        if value == SOURCE_LOCALE:
            return LOCALE
        value = value.replace("neoorigins_fra_", "neoorigins_fur_")
        value = re.sub(r"(?<=/)fra_de(?=\.json(?:$|[?#]))", LOCALE, value)
        value = re.sub(r"(?<=\\)fra_de(?=\.json(?:$|[?#]))", LOCALE, value)
        return value
    return value


def add_friulian_from_east_franconian(node) -> int:
    added = 0
    ...
    def walk(value):
        ...
        if isinstance(value, dict):
            if SOURCE_LOCALE in value and isinstance(value[SOURCE_LOCALE], dict):
                if LOCALE in value:
                    raise SystemExit("Friulian metadata already exists before locale 83 finalization")
...
def update_supported_counts(node) -> int:
    ...
    if isinstance(node, dict):
        for key, value in node.items():
            if key == "supported_locale_count" and value == 82:
                node[key] = 83
                changed += 1
            else:
...
def assert_friulian_entry_clean(value, project_id: str) -> None:
    if isinstance(value, dict):
        for item in value.values():
            assert_friulian_entry_clean(item, project_id)
    elif isinstance(value, list):
        for item in value:
            assert_friulian_entry_clean(item, project_id)
    elif isinstance(value, str):
        if "/fra_de.json" in value or "neoorigins_fra_" in value:
            raise SystemExit(
                f"{project_id}: East Franconian path/namespace leaked into Friulian metadata: {value!r}"
            )
...
def main() -> None:
    ...
    if project.get("supported_locale_count") != 82:
        raise SystemExit(
            "Expected catalog supported_locale_count=82 before Friulian finalization, "
            f"got {project.get('supported_locale_count')!r}"
        )

    added = add_friulian_from_east_franconian(catalog)
    if added != 11:
        raise SystemExit(f"Expected 11 Friulian catalog entries, got {added}")
    ...
    additions = {
        "friulian_common_glob": "neoorigins_fur_common_*",
        "friulian_mc_1_21_1": "neoorigins_fur_121",
    }
    ...
    if "All 82 currently supported locales" not in old_note:
        raise SystemExit("Unexpected NeoOrigins coverage_note before locale 83 finalization")
    ...
    readme = replace_exact(readme, "| 82 |", "| 83 |", expected=3)
    readme = replace_exact(
        readme,
        "**Francique oriental (`fra_de`)**.",
        "**Francique oriental (`fra_de`)** et **Frioulan (`fur_it`)**.",
        expected=1,
    )
    readme = replace_exact(
        readme,
    ...
    east_franconian_block = (
        "Pour le francique oriental :\n\n"
        "- **1.21.1 : 2 296/2 296 clés** couvertes ;\n"
        "- **26.1.x : 2 307/2 307 clés** couvertes ;\n"
        "- **26.2 : 2 307/2 307 clés** couvertes."
    )
    friulian_block = east_franconian_block + (
        "\n\nPour le frioulan :\n\n"
        "- **1.21.1 : 2 296/2 296 clés** couvertes ;\n"
        "- **26.1.x : 2 307/2 307 clés** couvertes ;\n"
        "- **26.2 : 2 307/2 307 clés** couvertes."
    )
    readme = replace_exact(readme, east_franconian_block, friulian_block, expected=1)
    ...
    catalog_md = replace_exact(
        catalog_md,
        "Fränggisch (`fra_de`)",
        "Fränggisch (`fra_de`) · Furlan (`fur_it`)",
        expected=11,
    )
    CATALOG_MD.write_text(catalog_md, encoding="utf-8")

    final_catalog = json.loads(CATALOG_JSON.read_text(encoding="utf-8"))
    if final_catalog["project"]["supported_locale_count"] != 83:
        raise SystemExit("Final supported_locale_count is not 83")
    for supported_project in final_catalog["supported_projects"]:
        ...
        if languages[LOCALE].get("name") != DISPLAY_NAME:
            raise SystemExit(
                f"{project_id}: unexpected Friulian display name {languages[LOCALE].get('name')!r}"
            )
        assert_friulian_entry_clean(languages[LOCALE], project_id)

    print("Metadata finalization passed: 82 -> 83 locales; fur_it added to all 11 projects.")
