#!/usr/bin/env python3
"""Refine Māori (`mi_nz`) fallbacks with direct NLLB English -> Māori MT."""
from __future__ import annotations
from pathlib import Path
import json,re
import torch
from transformers import AutoModelForSeq2SeqLM,AutoTokenizer
import bootstrap_maori as base

ROOT=base.ROOT;ASSETS=base.ASSETS
MODEL_ID="facebook/nllb-200-distilled-600M";SOURCE_LANG="eng_Latn";TARGET_LANG="mri_Latn"
...
def write_json(path:Path,data:dict):path.parent.mkdir(parents=True,exist_ok=True);path.write_text(json.dumps(data,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
...
def build_sources():
    ...
    addons={ns:read_json(ROOT/f"build/{folder}/upstream_en_us.json") for ns,folder in folders.items()};shared=set(addons["origins_backgrounds"]);addons["origins_backgrounds_two"]={k:v for k,v in addons["origins_backgrounds_two"].items() if k not in shared};addons["origins_backgrounds_iss"]={k:v for k,v in addons["origins_backgrounds_iss"].items() if k not in shared};return common,d121,d261,d262,addons
class Translator:
    def __init__(self):
        print(f"Loading {MODEL_ID}: {SOURCE_LANG}->{TARGET_LANG}",flush=True);self.tok=AutoTokenizer.from_pretrained(MODEL_ID,src_lang=SOURCE_LANG);self.model=AutoModelForSeq2SeqLM.from_pretrained(MODEL_ID);self.model.eval();self.target_id=self.tok.convert_tokens_to_ids(TARGET_LANG)
        if self.target_id is None or self.target_id==self.tok.unk_token_id:raise RuntimeError(f"Missing NLLB target {TARGET_LANG}")
    def batch(self,texts:list[str]):
        if not texts:return []
        enc=self.tok(texts,return_tensors="pt",padding=True,truncation=True,max_length=512)
        with torch.inference_mode():gen=self.model.generate(**enc,forced_bos_token_id=self.target_id,num_beams=2,max_new_tokens=512,early_stopping=True)
    ...
    def structural(self,source:str):
        ...
        if len(vals)!=len(indexes):raise RuntimeError("Māori structural span mismatch")
        for i,core,val in zip(indexes,cores,vals):pre,suf=aff[i];out[i]=pre+sanitize(core,val)+suf
        result=[]
        for i,piece in enumerate(out):result.append(piece);result.append(tokens[i]) if i<len(tokens) else None
        return "".join(result)
def main():
    before_files=sorted(ASSETS.glob("**/lang/mi_nz.json"))
    if len(before_files)!=29:raise SystemExit(f"Expected 29 Māori files, found {len(before_files)}")
    ...
    print(f"Māori pool: {len(unique)} unique; {len(unique)-len(direct)} corpus/manual; {len(direct)} direct NLLB",flush=True);mt=Translator()
    for start in range(0,len(direct),24):
        batch=direct[start:start+24];outputs=mt.batch(batch)
        if len(outputs)!=len(batch):raise SystemExit("Māori NLLB batch mismatch")
        for source,value in zip(batch,outputs):
            ...
            if not value:raise SystemExit(f"Empty Māori translation: {source!r}")
            if base.placeholder_signature(source)!=base.placeholder_signature(value):raise SystemExit(f"Māori placeholder mismatch: {source!r}->{value!r}")
            if protected_signature(source)!=protected_signature(value):raise SystemExit(f"Māori protected-token mismatch: {source!r}->{value!r}")
            if SUSPICIOUS_RE.search(value):raise SystemExit(f"Suspicious Māori output: {source!r}->{value!r}")
            translated[source]=value
        done=min(start+len(batch),len(direct))
        if done%240==0 or done==len(direct):print(f"Direct Māori progress: {done}/{len(direct)}",flush=True)
    def pl(src):return {k:translated[str(v)] for k,v in src.items()}
    common_mi=pl(common)
    for p in ASSETS.glob("neoorigins_mi_common_*/lang/mi_nz.json"):p.unlink()
    ...
    write_json(ASSETS/"neoorigins_mi_121/lang/mi_nz.json",pl(d121));write_json(ASSETS/"neoorigins_26_1/lang/mi_nz.json",pl(d261));write_json(ASSETS/"neoorigins_26_2/lang/mi_nz.json",pl(d262))
    for ns,src in addons.items():write_json(ASSETS/ns/"lang/mi_nz.json",pl(src))
    after_files=sorted(ASSETS.glob("**/lang/mi_nz.json"));after={p:read_json(p) for p in after_files};seen={};changed=total=0
    if len(after_files)!=29:raise SystemExit("Māori file count changed")
    for p,data in after.items():
        if set(before[p])!=set(data):raise SystemExit(f"Māori key set changed: {p}")
    ...
    if TARGET_LANG in text or SUSPICIOUS_RE.search(text):raise SystemExit("Māori target marker/artifact survived")
    if source_changed<2500:raise SystemExit(f"Too many English values survived Māori MT: {source_changed}")
    ...
    print(f"Māori refinement passed: {total} values / 29 files; {changed} bootstrap changes; {source_changed} source values changed",flush=True)
