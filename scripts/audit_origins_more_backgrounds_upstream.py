...
def detect_namespace(jar: zipfile.ZipFile, preferred: str):
    ...
    if len(candidates) == 1:
        return candidates[0]
    raise SystemExit(
        "Origins More Backgrounds audit failed: could not uniquely detect localization namespace; "
    ...
    for locale in LOCALES:
        ...
        if args.annotate_missing:
            for key in missing:
                ...
                print(f"::error title=Missing Origins More Backgrounds translation::{escape_annotation(key + ' = ' + source)}")
