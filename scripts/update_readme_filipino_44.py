...
replacements = {
    "`0.9.0-beta+1.21.1` | 21 | 43 |": "`0.9.0-beta+1.21.1` | 21 | 44 |",
    "`0.9.0-beta+26.1` | 25 | 43 |": "`0.9.0-beta+26.1` | 25 | 44 |",
    "`0.9.0-beta+26.2` | 25 | 43 |": "`0.9.0-beta+26.2` | 25 | 44 |",
    "**Islandais (`is_is`)** et **Malais (`ms_my`)**.": "**Islandais (`is_is`)**, **Malais (`ms_my`)** et **Filipino (`fil_ph`)**.",
    "Les quarante-trois langues sont disponibles": "Les quarante-quatre langues sont disponibles",
    "Couverture CS / HU / JA / KO / UK / ID / SV / DA / FI / NO / RO / EL / BG / VI / AR / HE / TH / SK / SL / HR / SR-CYR / SR-LAT / CA / ET / LT / LV / EU / GL / HI / NN / FA / IS / MS": "Couverture CS / HU / JA / KO / UK / ID / SV / DA / FI / NO / RO / EL / BG / VI / AR / HE / TH / SK / SL / HR / SR-CYR / SR-LAT / CA / ET / LT / LV / EU / GL / HI / NN / FA / IS / MS / FIL",
}
...
if "Pour le filipino :" not in text:
    if ms_block not in text:
        raise RuntimeError("Malay coverage block not found")
    text = text.replace(ms_block, fil_block, 1)
...
if "- **filipino** :" not in text:
    lines = text.splitlines()
    for i, line in enumerate(lines):
        if line.startswith(ms_jar_prefix):
            lines.insert(i + 1, fil_jar)
            break
    else:
        raise RuntimeError("Malay JAR validation line not found")
    text = "\n".join(lines) + "\n"

README.write_text(text, encoding="utf-8")
print(f"README.md updated for fil_ph / 44 locales ({fil_121_files} fallback files on 1.21.1), Malay preserved")
