...
replacements = {
    ...
    "**Persan (`fa_ir`)** et **Islandais (`is_is`)**.": "**Persan (`fa_ir`)**, **Islandais (`is_is`)** et **Malais (`ms_my`)**.",
    "Les quarante-deux langues sont disponibles": "Les quarante-trois langues sont disponibles",
    "Couverture CS / HU / JA / KO / UK / ID / SV / DA / FI / NO / RO / EL / BG / VI / AR / HE / TH / SK / SL / HR / SR-CYR / SR-LAT / CA / ET / LT / LV / EU / GL / HI / NN / FA / IS": "Couverture CS / HU / JA / KO / UK / ID / SV / DA / FI / NO / RO / EL / BG / VI / AR / HE / TH / SK / SL / HR / SR-CYR / SR-LAT / CA / ET / LT / LV / EU / GL / HI / NN / FA / IS / MS",
}
...
common_chunks = len(list(ASSETS.glob("neoorigins_ms_common_*/lang/ms_my.json")))
if common_chunks != 16:
    raise RuntimeError(f"Expected 16 Malay common chunks, found {common_chunks}")
for required in [
    ASSETS / "neoorigins_ms_121/lang/ms_my.json",
    ASSETS / "neoorigins_26_1/lang/ms_my.json",
    ASSETS / "neoorigins_26_2/lang/ms_my.json",
]:
    if not required.exists():
        raise RuntimeError(f"Missing Malay target delta: {required}")
...
addon_fallback_files = [
    ASSETS / namespace / "lang/ms_my.json" for namespace in addon_namespaces
    if (ASSETS / namespace / "lang/ms_my.json").exists()
]
ms_121_files = 17 + len(addon_fallback_files)

is_jar_prefix = "- **islandais** :"
ms_jar = f"- **malais** : {ms_121_files} fichiers fallback `ms_my` dans le JAR 1.21.1 et 17 dans chacun des JAR 26.x, avec priorité conservée aux traductions officielles amont ;"
if "- **malais** :" not in text:
    lines = text.splitlines()
    for i, line in enumerate(lines):
        if line.startswith(is_jar_prefix):
            lines.insert(i + 1, ms_jar)
            break
    else:
...
print(f"README.md updated for ms_my / 0.9.0-beta ({ms_121_files} fallback files on 1.21.1)")
