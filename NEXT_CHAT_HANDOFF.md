...
The first **85 selected locales are complete and integrated** into `release/1.0.0`, through **#85 Manx — `gv_im / Gaelg`**.
...
The catalogue and README are at **85 locales**. There are **28 selected locales remaining** for 1.0.0; the authoritative queue and exclusion policy are in `docs/RELEASE_1_0_0_LOCALE_PLAN.md`.

Do not redo completed locale metadata.

## Manx #85 — final references
...
- translation transport was switched to Google Translate Web `MkEWBc` RPC with mobile-page fallback after serialized endpoint attempts remained throttled
...
- metadata final state: **85 supported locales**, `gv_im` present across all 11 projects, 29 source files, native catalog name `Gaelg`, README French label `Mannois`, fallback namespaces `manx_common_glob = neoorigins_gv_common_*` and `manx_mc_1_21_1 = neoorigins_gv_121`
- public build version remains `0.9.0-beta`
- integration PR: `#22`
- integration merge commit on `release/1.0.0`: `4389ee8038fc176d4ce31b6ad7195967a4d8cdc3`
- locale-plan advancement commit: `51a62827b0cb477255be50df888d9c941e138667`
...
Start and finish **language #86 — Hawaiian (`haw_us`)** from the current updated `release/1.0.0` integration branch. Do not rerun Manx work.
