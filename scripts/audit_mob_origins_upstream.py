...
LOCALES = (
    "fr_fr", "de_de", "es_es", "pt_br", "nl_nl", "it_it", "pl_pl",
    "ru_ru", "tr_tr", "zh_cn", "cs_cz", "hu_hu", "ja_jp", "ko_kr",
    "uk_ua", "id_id", "sv_se",
)
DEFAULT_FILE_ID = "4827778"
DEFAULT_FILENAME = "moborigins-1.11.2.jar"
DEFAULT_NAMESPACE = "moborigins"
...
def download_jar(file_id: str, filename: str, attempts: int = 4):
    ...
    urls = (
        ...
        f"https://www.curseforge.com/minecraft/mc-mods/mob-origins/download/{file_id}/file",
    )
    last_error = None
    for attempt in range(1, attempts + 1):
        ...
        if attempt != attempts:
            delay = 2 ** (attempt - 1)
            print(f"Transient Mob Origins download error ({attempt}/{attempts}): {last_error}; retrying in {delay}s")
            time.sleep(delay)
    raise last_error
...
def detect_namespace(jar: zipfile.ZipFile, preferred: str):
    ...
    candidates = sorted(
        ...
        and name.endswith("/lang/en_us.json")
        and len(name.split("/")) == 4
    )
    if len(candidates) == 1:
        return candidates[0]
    raise SystemExit(
        f"Mob Origins audit failed: could not uniquely detect localization namespace; candidates={candidates}"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare Mob Origins fallback locales with the pinned upstream CurseForge JAR."
    )
    parser.add_argument("--file-id", default=DEFAULT_FILE_ID)
    parser.add_argument("--filename", default=DEFAULT_FILENAME)
    parser.add_argument("--namespace", default=DEFAULT_NAMESPACE)
    parser.add_argument("--output", default=str(ROOT / "build/mob-origins-upstream-audit"))
    ...
    with zipfile.ZipFile(io.BytesIO(jar_data)) as jar:
        ...
        if not en:
            raise SystemExit("Mob Origins audit failed: upstream en_us.json is missing")
        official_locales = {locale: read_locale(jar, namespace, locale) for locale in LOCALES}

    (out_dir / "upstream_en_us.json").write_text(
        json.dumps(en, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )
    pack_lang = PACK_ROOT / namespace / "lang"
    report = {
        "upstream": "Mob Origins",
        "curseforge_project_id": 449974,
        ...
        if args.annotate_missing:
            for key in missing:
                if key not in annotation_keys:
                    ...
                    print(
                        f"::error title=Missing Mob Origins translation::{escape_annotation(key + ' = ' + str(en.get(key, '')))}"
                    )

    (out_dir / "report.json").write_text(
        json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )
    print(f"Mob Origins audit ({args.filename}, CurseForge file {args.file_id})")
    ...
    if failures:
        raise SystemExit("Mob Origins audit failed: " + "; ".join(failures))
