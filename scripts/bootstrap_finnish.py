#!/usr/bin/env python3
"""Generate Finnish fallback localization from pinned English audit payloads.

The translation cache is kept under build/ so interrupted CI runs can resume within
one job. Official upstream Finnish strings are pruned by the audit pass after this
...
CACHE_PATH = ROOT / "build/finnish_translation_cache.json"
...
# generic machine pass. Extend only when a targeted review justifies it.
MANUAL_OVERRIDES = {
    "Open Origin Creator": "Avaa Origin-luontityökalu",
    "Mob Origin Creator": "Mob Origin -luontityökalu",
    "Split": "Jaa",
    "Elytra Boost": "Elytra-tehostus",
    "Sonic Boom": "Äänipamaus",
    "Pack Bond": "Laumaside",
    "Speed Mining": "Nopea louhinta",
    "Camoflauge": "Naamioituminen",
    "Pack Boost": "Laumatehostus",
    "Max Mana Boost": "Maksimimanan tehostus",
    "NeoOrigins: Grant Loot Pool": "NeoOrigins: Myönnä saalisvaranto",
    "Rage Counter.": "Raivomittari.",
    "Rage Counter": "Raivomittari",
...
def translate_rpc(text: str, attempts: int = 5):
    inner = json.dumps([[text, "en", "fi", True], [None]])
...
def translate_values(values: list[str]):
    ...
    print(f"Finnish cache: {len(cache)} entries; {len(pending)} new strings in {len(batches)} batches")
...
def main():
    neo_121 = read_json(ROOT / "build/fi-discovery-mc-1.21.1/fi_fi_missing_en.json")
    neo_261 = read_json(ROOT / "build/fi-discovery-mc-26.1/fi_fi_missing_en.json")
    neo_262 = read_json(ROOT / "build/fi-discovery-mc-26.2/fi_fi_missing_en.json")
    ...
    for index in range(common_chunks):
        ...
        write_json(ASSETS / f"neoorigins_fi_common_{index + 1:02d}/lang/fi_fi.json", translated(chunk))

    write_json(ASSETS / "neoorigins_fi_121/lang/fi_fi.json", translated(delta_121))
    write_json(ASSETS / "neoorigins_26_1/lang/fi_fi.json", translated(delta_261))
    write_json(ASSETS / "neoorigins_26_2/lang/fi_fi.json", translated(delta_262))
    for namespace, payload in addons.items():
        write_json(ASSETS / namespace / "lang/fi_fi.json", translated(payload))

    print(
        "Generated Finnish locale: "
