#!/usr/bin/env python3
"""Generate Interslavic (`isv`) fallback localization bootstrap files.

English remains the semantic source. Exact whole-string replacements are reused from
the pinned Minecraft Java `en_us` <-> `isv` corpus. Remaining English strings are
left intact for a dedicated full-sentence refinement pass. No isolated-word projection
is performed. This is automated assistance, not native-speaker review.
"""
...
def fetch_json(url: str):
    request = urllib.request.Request(url, headers={"User-Agent": "NeoOrigins-Interslavic-Bootstrap"})
...
def build_corpus_map() -> dict[str, str]:
    ...
    interslavic = fetch_json(f"{MC_LOCALES_BASE}/isv.json")
    shared = sorted(set(english) & set(interslavic))
    exact_votes: dict[str, Counter[str]] = defaultdict(Counter)
    rejected = unchanged = 0
    for key in shared:
        source = str(english[key])
        target = str(interslavic[key])
        if source.casefold() == target.casefold():
            unchanged += 1
            continue
    ...
    for source, counter in exact_votes.items():
        ...
        if top / total >= 0.80:
            ...
            ambiguous += 1
    print(
        f"Minecraft Interslavic corpus: {len(shared)} aligned entries, {rejected} rejected pairs, "
        f"{unchanged} unchanged English pairs, {len(exact)} exact Interslavic strings, "
        f"{ambiguous} ambiguous exact sources"
    )
    return exact


def interslavicize(text: str, exact: dict[str, str]) -> str:
    ...
    if placeholder_signature(text) != placeholder_signature(result):
        raise RuntimeError(f"Interslavic placeholder mismatch: {text!r} -> {result!r}")
    return result


def translated_payload(source: dict[str, str], exact: dict[str, str]):
    return {key: interslavicize(str(value), exact) for key, value in source.items()}
...
def main():
    ...
    common_en = {k: v for k, v in neo_121_en.items() if neo_261_en.get(k) == v and neo_262_en.get(k) == v}
    delta_121_en = {k: v for k, v in neo_121_en.items() if k not in common_en}
    delta_261_en = {k: v for k, v in neo_261_en.items() if k not in common_en}
    delta_262_en = {k: v for k, v in neo_262_en.items() if k not in common_en}
    print(f"NeoOrigins Interslavic split: common={len(common_en)}, 1.21.1={len(delta_121_en)}, 26.1={len(delta_261_en)}, 26.2={len(delta_262_en)}")
    ...
    addons_en["origins_backgrounds_two"] = {k: v for k, v in addons_en["origins_backgrounds_two"].items() if k not in shared_background_keys}
    addons_en["origins_backgrounds_iss"] = {k: v for k, v in addons_en["origins_backgrounds_iss"].items() if k not in shared_background_keys}

    exact = build_corpus_map()
    common_isv = translated_payload(common_en, exact)
    delta_121_isv = translated_payload(delta_121_en, exact)
    delta_261_isv = translated_payload(delta_261_en, exact)
    delta_262_isv = translated_payload(delta_262_en, exact)
    ...
    for index in range(chunks):
        write_json(ASSETS / f"neoorigins_isv_common_{index + 1:02d}/lang/isv.json", dict(common_items[index * chunk_size:(index + 1) * chunk_size]))
    ...
    for namespace, english in addons_en.items():
        write_json(ASSETS / namespace / "lang/isv.json", translated_payload(english, exact))

    files = sorted(ASSETS.glob("**/lang/isv.json"))
    if len(files) != 29:
        raise RuntimeError(f"Expected 29 Interslavic files after generation, found {len(files)}")

    source_payloads = [common_en, delta_121_en, delta_261_en, delta_262_en, *addons_en.values()]
    exact_hits = sum(1 for payload in source_payloads for value in payload.values() if str(value) in exact or str(value) in MANUAL_VALUES)
    total_values = sum(len(payload) for payload in source_payloads)
    remaining_english = total_values - exact_hits
    print(
        f"Interslavic bootstrap generated {len(files)} files; exact/manual replacements applied to {exact_hits}/{total_values} source values; "
        f"{remaining_english} values remain for direct full-sentence refinement."
    )
