#!/usr/bin/env python3
"""Generate Ido (`io_en`) fallback localization bootstrap files.

English remains the semantic source. Exact whole-string replacements are reused from
the pinned Minecraft Java `en_us` <-> `io_en` corpus. Remaining English strings are
...
def fetch_json(url: str):
    request = urllib.request.Request(url, headers={"User-Agent": "NeoOrigins-Ido-Bootstrap"})
...
def build_corpus_map() -> dict[str, str]:
    ...
    ido = fetch_json(f"{MC_LOCALES_BASE}/io_en.json")
    shared = sorted(set(english) & set(ido))
    ...
    for source, counter in exact_votes.items():
        ...
        if top / total >= 0.80:
            ...
            ambiguous += 1
    print(
        f"Minecraft Ido corpus: {len(shared)} aligned entries, {rejected} rejected pairs, "
        f"{unchanged} unchanged English pairs, {len(exact)} exact Ido strings, "
        f"{ambiguous} ambiguous exact sources"
    )
    return exact


def idoize(text: str, exact: dict[str, str]) -> str:
    ...
    if placeholder_signature(text) != placeholder_signature(result):
        raise RuntimeError(f"Ido placeholder mismatch: {text!r} -> {result!r}")
    return result


def translated_payload(source: dict[str, str], exact: dict[str, str]):
    return {key: idoize(str(value), exact) for key, value in source.items()}
...
def main():
    ...
    print(f"NeoOrigins Ido split: common={len(common_en)}, 1.21.1={len(delta_121_en)}, 26.1={len(delta_261_en)}, 26.2={len(delta_262_en)}")
    ...
    common_io = translated_payload(common_en, exact)
    delta_121_io = translated_payload(delta_121_en, exact)
    delta_261_io = translated_payload(delta_261_en, exact)
    delta_262_io = translated_payload(delta_262_en, exact)
    ...
    for index in range(chunks):
        write_json(ASSETS / f"neoorigins_io_common_{index + 1:02d}/lang/io_en.json", dict(common_items[index * chunk_size:(index + 1) * chunk_size]))
    ...
    for namespace, english in addons_en.items():
        write_json(ASSETS / namespace / "lang/io_en.json", translated_payload(english, exact))

    files = sorted(ASSETS.glob("**/lang/io_en.json"))
    if len(files) != 29:
        raise RuntimeError(f"Expected 29 Ido files after generation, found {len(files)}")
    ...
    print(
        f"Ido bootstrap generated {len(files)} files; exact/manual replacements applied to {exact_hits}/{total_values} source values; "
